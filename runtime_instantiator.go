...
// IRuntimeInstantiator 为 RuntimeAssembler 提供自定义 IRuntime 实例。
// 每次构建运行时都会调用一次 Instantiate，且返回值必须是新的非 nil 实例。
...
func newRuntimeInstantiator(rtInst any) *_RuntimeInstantiator {
	...
	if rtInstRT.PkgPath() == "" || rtInstRT.Name() == "" || !reflect.PointerTo(rtInstRT).Implements(reflect.TypeFor[IRuntime]()) {
