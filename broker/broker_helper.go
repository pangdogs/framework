...
func Subscribe(serviceCtx service.Context, ctx context.Context, pattern string, settings ...option.Setting[SubscriberOptions]) (Subscriber, error) {
	return Using(serviceCtx).Subscribe(ctx, pattern, settings...)
}

// SubscribeSync will express interest in the given topic pattern.
func SubscribeSync(serviceCtx service.Context, ctx context.Context, pattern string, settings ...option.Setting[SubscriberOptions]) (SyncSubscriber, error) {
	return Using(serviceCtx).SubscribeSync(ctx, pattern, settings...)
}

// SubscribeChan will express interest in the given topic pattern.
func SubscribeChan(serviceCtx service.Context, ctx context.Context, pattern string, settings ...option.Setting[SubscriberOptions]) (ChanSubscriber, error) {
	return Using(serviceCtx).SubscribeChan(ctx, pattern, settings...)
}
