...
type (
	...
	SessionStateChangedHandler = func(session Session, old, new SessionState)                                    // 会话状态变化的处理器
	SessionRecvDataHandler     = func(session Session, data []byte) error                                        // 会话接收的数据的处理器
	SessionRecvEventHandler    = func(session Session, event transport.Event[gtp.Msg]) error                     // 会话接收的自定义事件的处理器
)
...
func (_GateOption) SessionStateChangedHandlers(handlers ...SessionStateChangedHandler) GateOption {
	return func(options *GateOptions) {
...
func (_GateOption) SessionRecvDataHandlers(handlers ...SessionRecvDataHandler) GateOption {
	return func(options *GateOptions) {
