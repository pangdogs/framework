...
// IMongoDB 按 tag 提供已连接的 MongoDB 客户端及其反射值。
type IMongoDB interface {
	// DB 返回 tag 对应的客户端；不存在时返回 nil。
	DB(tag string) *mongo.Client
	// ReflectedDB 返回 tag 对应客户端的反射值；不存在时返回无效值。
	ReflectedDB(tag string) reflect.Value
}

// DB 返回 svcCtx 中 MongoDB add-in 按 tag 注册的客户端；add-in 未安装时会 panic。
